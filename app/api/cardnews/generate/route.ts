...
2. card1.title_line1 상호명 표기 규칙 (반드시 준수):
   - 상호명 전체를 자연스러운 번체 중문으로만 표기 가능하면 → 번체 중문으로만 표기 (예: 토라슌 → 吐拉順)
   - 번체로 변환 시 한글이 한 글자라도 섞이면 → 영문 음역으로만 표기 (예: 大廳마루 ✗ → Daecheong Maru ✓)
   - 결과물은 반드시 순수 번체 중문 또는 순수 영문 중 하나여야 함. 절대 혼용 금지
...

{
  ...
  "card1": {
    ...
    "title_line1": "순수 번체 중문 또는 순수 영문 음역 (한글 혼용 절대 금지)",
