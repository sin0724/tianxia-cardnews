...
아래 형식으로 블로그 원고를 JSON으로 출력하세요 (설명 없이 JSON만):
{
  ...
  "content": "본문 전체 (순수 텍스트, 총 1200~1800자)",
  "tags": ["태그1", "태그2", "태그3", "태그4", "태그5"]
}

content 작성 규칙 (반드시 준수):
- #, ##, ###, **, __, * 등 마크다운 기호를 절대 사용하지 마세요
- 소제목은 꺾쇠 없이 줄 바꿈 후 짧게 쓰세요 (예: "1. 대만 MZ세대의 특징")
- 굵게 강조할 내용은 기호 없이 문장 안에 자연스럽게 녹여 쓰세요
- 목록은 "- " 대신 "· " 또는 번호(1. 2. 3.)를 사용하세요
- 단락 구분은 빈 줄(\n\n)로 하세요

작성 가이드라인:
- 독자: 대만 시장 진출을 검토 중인 한국 브랜드 마케팅 담당자
- 톤: 전문적이지만 읽기 쉽게, 실용적인 인사이트 중심
- 구성: 도입부 → 핵심 내용 3~4파트 → 실행 가능한 팁 → 마무리
- 각 파트 2~3 단락
- 구체적인 수치나 사례 포함
